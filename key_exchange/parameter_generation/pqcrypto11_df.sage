...
weights = {2 : (5, 2), 3 : (2, 1), 5 : (1, 1), 7 : (1,1),
...
def ss_isogeny_df(*args, **kw):
    ...
    res = ss_isogeny_exchange(*params, measure='measure' in kw and kw['measure'])
    res = ss_isogeny_exchange_df(*params, measure='measure' in kw and kw['measure'])
...
def ss_isogeny_gen(lA, lB, eA, eB, f, pm1):
   ...
   #D = -2404#hardcoding D=-2404 for testing purposes
    ...
    if ((pm1 == -1 and not (E.A + 2).is_square) or
        ...
        E = E.quadratic_twist()
    misc.verbose(t=ct)
    misc.verbose("D_Elliptic Curve: %s" % E)  
    misc.verbose()
...
def ss_isogeny_exchange(E, lA, eA, PA, QA, strA, lB, eB, PB, QB, strB, measure=None):
    ...
    def rand_subgroup(l, e):
        if randrange(0, l+1):                                      #randrange is sage needs to be changes
	...
	#temporarily hardcode precomputations for testing
   ...
   # phiQB.printPoint()
    ...
    misc.verbose("EsA: %s"%EsA)
    misc.verbose("J-Invariant of EsA %s"%EsA.j_invariant())
    misc.verbose(t=ct)
    misc.verbose()

    if EsA.j_invariant() != EsB.j_invariant():                                    #j_invariant() is sage i think - this should be easy to change
...
#generates and writes specified parameters to file
def ss_isogeny_gen_file(lA, lB, eA, eB, f, pm1):

     prime = "%s^%s * %s^%s" % (lA, eA, lB, eB) + (f != 1)*(" * %s" % f) + (pm1 == 1)*" + 1" + (pm1 == -1)*" - 1"
     p = sage_eval(prime)

     name = "%d_%d_%d_%d_%d_%d"%(lA,lB,eA,eB,f,pm1)
     nums = "%s\n%d\n%d\n%d\n%d\n"%(p,lA,lB,eA,eB)
     params = ss_isogeny_gen(lA, lB, eA, eB, f, pm1)

     
     mPA = params[3].getMPoint()
     mQA = params[4].getMPoint()
     mPB = params[8].getMPoint()
     mQB = params[9].getMPoint()
     ...
     print("%s",mPA.getXaRep());
    
     mpa = "%s\n%s\n%s\n%s\n%s\n%s\n"%(mPA.getXaRep(), mPA.getXbRep(), mPA.getYaRep(), mPA.getYbRep(), mPA.getZaRep(), mPA.getZbRep())
     
     mqa = "%s\n%s\n%s\n%s\n%s\n%s\n"%(mQA.getXaRep(), mQA.getXbRep(), mQA.getYaRep(), mQA.getYbRep(), mQA.getZaRep(), mQA.getZbRep())
     mpb = "%s\n%s\n%s\n%s\n%s\n%s\n"%(mPB.getXaRep(), mPB.getXbRep(), mPB.getYaRep(), mPB.getYbRep(), mPB.getZaRep(), mPB.getZbRep())
     mqb = "%s\n%s\n%s\n%s\n%s\n%s\n"%(mQB.getXaRep(), mQB.getXbRep(), mQB.getYaRep(), mQB.getYbRep(), mQB.getZaRep(), mQB.getZbRep())
     curve ="%s\n%s\n%s\n%s\n%s\n%s\n"%(mPA.getCurveAa(), mPA.getCurveAb(), mPA.getCurveBa(), mPA.getCurveBb(), mPA.getCurveA24a(), mPA.getCurveA24b())
     ...
     #precomputations for Barrett reduction     
     k = 0;
     while 2**k < p:
        k = k+1
