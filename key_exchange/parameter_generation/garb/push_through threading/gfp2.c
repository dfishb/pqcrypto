...
void push_through_iso(GF *A, GF *B, GF *A24,
  ...
  GF_params* field = A->parent;
  int r, split, i, j, k, len, len1, len2, first = 1;
  union isogenies phi;
  ...
  if(len%2==0){
        ...
        len1=floor(len*1.0/2);
        //len2=floor(len*1.0/2)+1;
  }
  ...
  for( j=0; j<len; j++){
    ...
    while (h > 1) {
      ...
      copy_GF(&tmp->z, tail->z);
        ...
        //multiplications by ell. split indicates where the path splits. h-split = height - point where path splits
		for ( i=0 ; i < h - split ; i++) {	  
	if (ell == 2){
    ...
    // Compute and apply the isogeny. The number of times this is computed corresponds to eA and eB. Note that this is 
      //computing/evaluating the isogeny based on computing the intermediate isogenys on the bottom of the pyramid as specifed on p. 8 of the paper
      //Note that for l=2 only e-2 isogenys will be computed using the method in this function since we add a degree 4 isogeny at the end
      ////Note when after ths is called, tmp=tail and tail is now the previous element in the stack
      //phi.d2 is adjusted in COMP_ISOG. 
      count3++;
      queue_point *tmp2 = malloc(sizeof(queue_point));
      init_QP(tmp2, field);
      
    if (ell == 2) { 
      COMP_ISOG(iso2_comp, phi.d2);

    //applying the isogenies for rightward movement on the tree. computes 1 isogeny to the right of all elements currently in the stack? i.e. all the forks  
   
      //  print_GF(phi.d2,"\nphi.d2");
        //this is what should be //ized
        for ( tmp = tail ; tmp ; tmp = Q_PREV(tmp)) {	
            copy_QP(tmp2, *tmp);
            ...
            iso2_apply(&tmp->x, NULL, &tmp->z,  phi.d2,		
                       tmp->x, tmp->x, tmp->z);			
            tmp->h = tmp->h - 1;				
        }
        ...
        */
            ...
            if (Px && Py && Pz){  
                iso2_apply(Px, Py, Pz, phi.d2, *Px, *Py, *Pz);
            }                           
            if (Qx && Qy && Qz)					
                iso2_apply(Qx, Qy, Qz, phi.d2, *Qx, *Qy, *Qz);		
        ...
        COMP_ISOG(iso3_comp, phi.d3);
            ...
            for ( tmp = tail ; tmp ; tmp = Q_PREV(tmp)) {	
               ...
               iso3_apply(&tmp->x, NULL, &tmp->z,  phi.d3,		
                           tmp->x, tmp->x, tmp->z);			
                tmp->h = tmp->h - 1;				
            }
    ...
    //    Q_POP(t->qp3, tmp);
      ...
      /*  for ( tmp=tail; tmp ; tmp = Q_NEXT(tmp)){	
           // copy_QP(tmp2, *tmp);
           ...
           // Q_PUSH(t->qp3, tmp2);
            ...
            tmp->h = tmp->h - 1;
                ...
                if (Px && Py && Pz){  
                    iso3_apply(Px, Py, Pz, phi.d3, *Px, *Py, *Pz);
                }                           
                if (Qx && Qy && Qz)					
                    iso3_apply(Qx, Qy, Qz, phi.d3, *Qx, *Qy, *Qz);		
