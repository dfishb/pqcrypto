...
typedef struct{
    ...
    GF *x1, *z1, *x2, *z2, *x3, *z3, dx2, dz2, dx3, dz3, A24; 
    GF tmpA[GF_TMP_REGS];
    GF tmpB[GF_TMP_REGS];
    GF tmpC[GF_TMP_REGS];
    mpz_t tmp1[3];
...
pthread_cond_t p2_cond, p3_cond, p2_done, p3_done, p1_cond, p1_done;
pthread_mutex_t p2_mutex, p3_mutex, p2_mutex2, p3_mutex2, p1_mutex, p1_mutex2;
...
void *p3(){
  ...
  /*  int i=0;
    for(i; i<GF_TMP_REGS; i++)
        init_GF(&tmp[i], (t.x1)->parent);
    */
    
   // pthread_mutex_lock(&p3_mutex);
    
    while(countM==0)
        ;//pthread_cond_wait(&p3_cond, &p3_mutex);
   ...
   //   printf("\nbeforeWhile3\n");
    ...
    /*     add_GF_t(&tmp[4], *(t.x1), *(t.z1), tmpA);         // a = (self.x + self.z)
         sub_GF_t(&tmp[5], *(t.x1), *(t.z1), tmpA);         // b = (self.x - self.z)
         //sub_GF_t(&tmp[6], *(t.x2), *(t.z2), tmpA);
         //add_GF_t(&tmp[7], *(t.x2), *(t.z2), tmpA);
         sub_GF_t(&tmp[0], *(t.x3), *(t.z3), tmpA);
         add_GF_t(&tmp[1], *(t.x3), *(t.z3), tmpA);*/
         
         mul_GF_t(&(tmp[2]), tmp[0], tmp[4], tmpA); // da = (P.x - P.z)*a
         mul_GF_t(&(tmp[3]), tmp[1], tmp[5], tmpA); // cb = (P.x + P.z)*b
    
         add_GF_t(&(tmp[8]), tmp[2], tmp[3], tmpA);
         sqr_GF_t(&(tmp[0]), tmp[8], tmpA);
         mul_GF_t((t->x3), tmp[0], t->dz3, tmpA);     // x2 = diff.z*(da + cb).square()
    
         sub_GF_t(&(tmp[8]), tmp[2], tmp[3], tmpA);
         sqr_GF_t(&(tmp[0]), tmp[8], tmpA);
         mul_GF_t((t->z3), tmp[0], t->dx3, tmpA);     // z2 = diff.x*(da - cb).square()
  ...
  /*       if(count3==1){
    ...
    */     
       ...
       //   p3_signal = 1;
      //   pthread_mutex_unlock(&p3_mutex);
    //      printf("\p3_nfishbein1\n");
          pthread_mutex_lock(&p3_mutex2);
          pthread_cond_signal(&p3_done);
      //     printf("\p3_nfishbein2\n");        
          pthread_mutex_unlock(&p3_mutex2);
  ...
  //  printf("\nbeforeWhile2\n");
    ...
    /*    add_GF_t(&tmp[4], *(t.x1), *(t.z1), tmpA);         // a = (self.x + self.z)
       ...
       // add_GF_t(&tmp[1], *(t.x3), *(t.z3), tmpA);*/
        ...
        mul_GF_t(&tmp[6], tmp[6], tmp[4], tmpA); // da = (P.x - P.z)*a
        mul_GF_t(&tmp[7], tmp[7], tmp[5], tmpA); // cb = (P.x + P.z)*b
    
        add_GF_t(&(tmp[2]), tmp[6], tmp[7], tmpA);
        sqr_GF_t(&(tmp[3]), tmp[2], tmpA);
        mul_GF_t(t->x2, tmp[3], t->dz2, tmpA);     // x2 = diff.z*(da + cb).square()
    
        sub_GF_t(&(tmp[3]), tmp[6], tmp[7], tmpA);
        sqr_GF_t(&(tmp[8]), tmp[3], tmpA);
        mul_GF_t(t->z2, tmp[8], t->dx2, tmpA);     // z2 = diff.x*(da - cb).square()
...
*/        
   ...
   //     p2_signal = 1;
     ...
     //   pthread_mutex_unlock(&p2_mutex);
       //  printf("\p2_nfishbein1\n");
         pthread_mutex_lock(&p2_mutex2);
     //   printf("\p2_nfishbein2\n");
         pthread_cond_signal(&p2_done);
         pthread_mutex_unlock(&p2_mutex2);
  ...
  //  pthread_mutex_unlock(&p1_mutex);
   ...
   //   printf("\nafterWhile1\n");        
     //   p1_signal = 0;
        ...
        /* P1 */
        sqr_GF_t(&tmp[6], tmp[4], tmpA);         // aa = a.square()
        //    printf("\nfish15\n");
        sqr_GF_t(&tmp[7], tmp[5], tmpA);         // bb = b.square()
        sub_GF_t(&tmp[8], tmp[6], tmp[7], tmpA); // e = aa - bb
        mul_GF_t(&tmp[4], t->A24, tmp[8], tmpA);
        add_GF_t(&tmp[5], tmp[4], tmp[7], tmpA);
        //  printf("\nfish16\n");
        mul_GF_t(t->z1, tmp[5], tmp[8], tmpA);      // z1 = e*(bb + self.curve.A24*e))
        // printf("\nfish17\n");
        mul_GF_t(t->x1, tmp[6], tmp[7], tmpA);      // x1 = aa*bb
     ...
     //   p1_signal = 1;
      ...
      //  pthread_mutex_unlock(&p1_mutex);
      //  printf("\p2_nfishbein1\n");
        pthread_mutex_lock(&p1_mutex2);
        //printf("\p2_nfishbein2\n");
        pthread_cond_signal(&p1_done);
        pthread_mutex_unlock(&p1_mutex2);
       // printf("\p2_nfishbein3\n");
...
//threadsafe version of mont_tradd
void * mont_tradd_t() {
 ...
 //   int i=0;
  ...
  //      init_GF(&tmp[i], (t->x1)->parent);
    ...
    add_GF_t(&(t->tmpA[4]), *(t->x1), *(t->z1), tmpA);         // a = (self.x + self.z)
    sub_GF_t(&(t->tmpA[5]), *(t->x1), *(t->z1), tmpA);         // b = (self.x - self.z)
    sub_GF_t(&(t->tmpA[6]), *(t->x2), *(t->z2), tmpA);
    add_GF_t(&(t->tmpA[7]), *(t->x2), *(t->z2), tmpA);
    sub_GF_t(&(t->tmpA[0]), *(t->x3), *(t->z3), tmpA);
    add_GF_t(&(t->tmpA[1]), *(t->x3), *(t->z3), tmpA); 
    

    
    copy_GF(&(t->tmpC[0]), t->tmpA[0]);
    copy_GF(&(t->tmpC[4]), t->tmpA[4]);
    copy_GF(&(t->tmpC[1]), t->tmpA[1]);
    copy_GF(&(t->tmpC[5]), t->tmpA[5]);
    copy_GF(&(t->tmpB[7]), t->tmpA[7]);
    copy_GF(&(t->tmpB[4]), t->tmpA[4]);
    copy_GF(&(t->tmpB[6]), t->tmpA[6]);
    copy_GF(&(t->tmpB[5]), t->tmpA[5]);
        pthread_mutex_lock(&p1_mutex);
   // pthread_cond_broadcast(&p1_cond);
    countM++;
    //printf("\nentered\n");
  ...
  */
  ...
  /*  if( count3< count2 && count3<count1){
        pthread_mutex_lock(&p3_mutex2);
        pthread_cond_wait(&p3_done, &p3_mutex2);
        pthread_mutex_unlock(&p3_mutex2);
    }
    
    if( count2< count3 && count2<count1){
        pthread_mutex_lock(&p2_mutex2);
        pthread_cond_wait(&p2_done, &p2_mutex2);
        pthread_mutex_unlock(&p2_mutex2);
    }
    
    if( count1< count3 && count1<count2){
        pthread_mutex_lock(&p1_mutex2);
        pthread_cond_wait(&p1_done, &p1_mutex2);
        pthread_mutex_unlock(&p1_mutex2);
    }
*/
...
void mont_3ladder(GF* Rx, GF* Rz,
  ...
  int first=0;  
  GF Rx1, Rz1;
  int bit = mpz_sizeinbase(tr, 2) - 1;
    
  bitCount++;  
  for ( ; bit >=0 ; bit--) {
      
    //pthread_t thread[bit+1];
      
      if(bitCount==1)
          countLadder++;
      
    if (mpz_tstbit(tr, bit) == 0) {
    ...
    /*    if(count1==1){
     ...
     //   pthread_mutex_unlock(&p3_mutex);
      ...
      //  t = {&tmp[2], &tmp[3], &tmp[0], &tmp[1], &Rx, &Rz, Qx, Qz, QPx, QPz, A24};
...
void push_through_iso(GF *A, GF *B, GF *A24,
  ...
  if (ell == 2) {
    ...
    do {		
        for ( tmp = tail ; tmp ; tmp = Q_PREV(tmp)) {	
            iso4_apply(&tmp->x, NULL, &tmp->z, phi.d4,		
                  tmp->x, tmp->x, tmp->z);			
            tmp->h = tmp->h - 2;				
        }
        ...
        if (Px && Py && Pz){  
            iso4_apply(Px, Py, Pz, phi.d4, *Px, *Py, *Pz);
            //thread_p2 thread_params = {Px, Py, Pz, *Px, *Py, *Pz, phi.d4}; 
           // rc = pthread_create(&thread1, NULL, apply_t, (void *) &thread_params); 
            
            
        }                           
        if (Qx && Qy && Qz)					
            iso4_apply(Qx, Qy, Qz, phi.d4, *Qx, *Qy, *Qz);	
...
void keygen_c_dfc1(MP *Pother2, MP *Qother2, mpz_t M, mpz_t N, int ell, int *strat, int len, MP P, MP Q, MP Pother, MP Qother, MP QP, MP PQ){
...
double ss_isogeny_exchange_dfc(double *time, char * eA, char * eB, char * lA_str, char * lB_str, int *strA, int lenA, int *strB, int lenB, MP *PA, MP *QA, MP *PB, MP *QB){
    ...
    t->x1 = malloc(sizeof(GF));
    t->x2 = malloc(sizeof(GF));
    t->x3 = malloc(sizeof(GF));
    t->z1 = malloc(sizeof(GF));
    t->z2 = malloc(sizeof(GF));
    t->z3 = malloc(sizeof(GF));
    ...
    init_GF( &(t->dx2), parent );
    init_GF( &(t->dz2), parent );
    init_GF( &(t->dx3), parent );
    init_GF( &(t->dz3), parent );
    init_GF( &(t->A24), parent );
    ...
    int i=0;
    for(i; i<GF_TMP_REGS; i++)
        init_GF(&(t->tmpA[i]), parent);
    
    int k=0;
    for(k; k<GF_TMP_REGS; k++)
        init_GF(&(t->tmpB[k]), parent);
    
    int j=0;
    for(j; j<GF_TMP_REGS; j++)
        init_GF(&(t->tmpC[j]), parent);

    printf("----------------Generating Alice's Public Data\n");
    gettimeofday(&tv1,NULL);
    keygen_c_dfc1(&phiPB, &phiQB, *mA, *nA, lA, strA, lenA, *PA, *QA, *PB, *QB, QPA, PQA);
    gettimeofday(&tv2,NULL);
    printf("----------------Generating Bob's Public Data\n");
    gettimeofday(&tv3,NULL);
    keygen_c_dfc1(&phiPA, &phiQA, *mB, *nB, lB, strB, lenB, *PB, *QB, *PA, *QA, QPB, PQB);
...
int main(int argc, char *argv[]) {
    ...
    for(i; i<iterations; i++){
        ...
        pthread_cond_init(&p3_cond, NULL);
        pthread_cond_init(&p1_done, NULL);
        ...
        pthread_mutex_init(&p2_mutex2, NULL); 
        pthread_mutex_init(&p3_mutex2, NULL); 
