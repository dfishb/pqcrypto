...
void neg_GF(GF *res, const GF x) {
    if (mpz_sgn(x.a) == 0)
        ...
        if(dieter==1 && count==1){
            gmp_printf("resa_1: %Zd\n\n", res->a);
            gmp_printf("p_1: %Zd\n\n",  x.parent->p);
            gmp_printf("x.a_1: %Zd\n\n",  x.a);
            gmp_printf("x.b_1: %Zd\n\n",  x.b);
            gmp_printf("x.tmp1: %Zd\n\n",  x.parent->tmp1);
            gmp_printf("x.tmp2: %Zd\n\n",   x.parent->tmp2);
            gmp_printf("x.tmp3: %Zd\n\n",   x.parent->tmp3);
        
        }
        mpz_sub(res->a, x.parent->p, x.a); 
        if(dieter==1 && count==1){
            count++;
            gmp_printf("resa_2: %Zd\n\n", res->a);
            gmp_printf("p_2: %Zd\n\n",  x.parent->p);
            gmp_printf("x.a_2: %Zd\n\n",  x.a);
            print_GF(*res,"res");
            print_GF(x,"x");
...
void iso4_apply(GF* X, GF* Y, GF* Z,
    ...
    if (Y) {
        ...
        if (mpz_sgn(x.b) == 0)
            ...
            mpz_sub(tmp[7].b, tmp[7].parent->p, x.b);
        
        
        
        dieter=0;
        printf("neg_2\n\n");
        print_GF(tmp[7], "tmp[7] - into neg_GF -");
...
double ss_isogeny_exchange_dfc(double *time, char * eA, char * eB, char * lA_str, char * lB_str, int *strA, int lenA, int *strB, int lenB, MP *PA, MP *QA, MP *PB, MP *QB){
    ...
    lB = atoi(lB_str);
     ...
     mpz_set_str(mA, "462423596882212386166055053023569254226149967282422493015916547851801311332224",10);
     mpz_set_str(nA, "1",10);
     mpz_set_str(mB, "46089853033926767106703880106602793967352301147078008843556109830524519650383",10);
     mpz_set_str(nB, "1",10);
...
int main(int argc, char *argv[]) {
    ...
    ss_isogeny_exchange_dfc(time, eA, eB, lA, lB, strA, lenA, strB, lenB, PA, QA, PB, QB);
 ...
 /*   
    ...
    GF x, y, z;
    iso4 *iso;
    iso = malloc(sizeof(iso4));
    GF_params* parent;
    parent = malloc(sizeof(GF_params));
    setup_GF(parent,"5646428529833603710854376801719732121889771686125102421349898409102848702003905102057129871853579459735758942656769724874115169484320460874488881525976727551");

    init_GF(&x, parent);
    init_GF(&y, parent);
    init_GF(&z, parent);
    init_GF(&(iso->Ap2),parent);
    
    set_GF(&x, "2088108437706083776238233801330717845539080563649334065657285002107172747089736946664672588678256990191472534640227147985355070843150638727516591558156321358", "4454065323750003791676117671672364601752409389245040994322060668653808658052810773715959911258392593872140193555334433395726189075638704274560197796176754058");
    set_GF(&y, "1449207419216337684650023604459925114021191449709399102174610763077208073064104291938028930698361361445131974968432504831289541806119282772453965931408326885", "1437060651280342825071195300379133797727397301536870473809436357141260071570596413930256343817958084759289755999996527802772428626277644845065711693404855548");
    set_GF(&z, "1428015607367260415748753951877047801354145939099138747186505093453745046442242124570595716325613886923239915460358647431962596738050270553455205901075886878", "5253177093521495051738763579850504262356571661299051809069787126857678000567742760971430740781210210921925713613293766104938739455980551102752948990674368590");
    set_GF(&(iso->Ap2), "3889971223092035956855715395672508216520195017729518855178057321505739156265372156588302464194940016281509805326794327563497446241409050860431783022809246807", "4082178128280982844610363039249103817312978507681215955291581247844641059099628350083057735893400866534594127949574262516234412796929366574421498244496606694");
    ...
    set_GF(&tmp[7], "3889971223092035956855715395672508216520195017729518855178057321505739156265372156588302464194940016281509805326794327563497446241409050860431783022809246807",  "4082178128280982844610363039249103817312978507681215955291581247844641059099628350083057735893400866534594127949574262516234412796929366574421498244496606690");
    mpz_set_str (x.parent->tmp1, "4082178128280982844610363039249103817312978507681215955291581247844641059099628350083057735893400866534594127949574262516234412796929366574421498244496606690", 10);
    mpz_set_str (x.parent->tmp2, "-", 10);
    mpz_set_str (x.parent->tmp3, "-1563765473869304451941612635715801899650349996012759846830980205705394670051723069898113612696066908403303536914408555290246782864467489387526077003022661901", 10);
    
    count=1;
    iso4_apply( &x, &y, &z, *iso, x, y, z); 
